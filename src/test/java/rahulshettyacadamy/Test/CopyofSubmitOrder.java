...
public class CopyofSubmitOrder extends BaseTest{
	...
	public void SubmitOrder() throws InterruptedException, IOException {
		 ...
		 */
//instaed of passing productcatalogue here pass it in landing page which is just before productcata page
		 ...
		 * () .orElse(null);
		 */
		// String price = product.findElement(By.cssSelector(".card-body
		// .text-muted")).getText();
		// product.findElement(By.cssSelector("button[class='btn w-10
		// rounded']")).click();
		...
		/*
		 * checkout.selectCardDetail(); checkout.PlaceOrder();
		 */
		
		// List<WebElement> CartProducts =
		// driver.findElements(By.cssSelector(".cartSection h3"));
		// List<WebElement> cartProductPrice =
		// driver.findElements(By.cssSelector(".prodTotal p"));
		...
		/*
		 * boolean Actualname = CartProducts.stream().anyMatch(s ->
		 * s.getText().equalsIgnoreCase(productName)); Assert.assertTrue(Actualname);
		 * boolean ActualPrice = cartProductPrice.stream().anyMatch(s ->
		 * s.getText().equalsIgnoreCase(price)); Assert.assertTrue(ActualPrice);
		 */
		 ...
		 * ".payment"))); WebElement SendCountry =
		 * driver.findElement(By.cssSelector("input[placeholder*='Country']")); Actions
		 * a = new Actions(driver); // new way to send key using action first is elemnet
		 * second value to pass a.sendKeys(SendCountry, Country).build().perform();
		 * List<WebElement> Countries =
		 * driver.findElements(By.cssSelector(".list-group-item span"));
		 */
		/*
		...
		/*
		 * Countries.stream().filter(s ->
		 * s.getText().toLowerCase().contains(Country.toLowerCase())).findFirst()
		 * .ifPresent(s -> s.click());
		 */
		 ...
		 * driver.findElement(By.xpath("//a[contains(@class,'action__submit')]")); try {
		 * Placeholder.click(); System.out.println("Click through Automation");
		 * 
		 * } catch (Exception e) { js.executeScript("arguments[0].click();",
		 * Placeholder);
